...
impl AIProvider for CohereProvider {
    ...
    async fn health_check(&self) -> Result<HealthCheck, ProviderError> {
        ...
        if self.config.api_key.is_none() {
            return Ok(HealthCheck {
                ...
                error_message: Some("Cohere API key not configured".to_string()),
            });
        }
    ...
    async fn complete(&self, request: CompletionRequest) -> Result<CompletionResponse, ProviderError> {
        info!("Cohere completion request for model: {:?}", request.model);
    ...
    fn estimate_cost(&self, request: &CompletionRequest) -> Option<f64> {
        ...
        let model = request.model.as_ref().unwrap_or(&"command-r-plus".to_string());
